...
pub fn learn_deref() {
    ...
    // 以上代码有几点值得注意:
    // 1. 要实现 DerefMut 必须要先实现 Deref 特征：pub trait DerefMut: Deref
    // 2. T: DerefMut<Target=U> 解读：将 &mut T 类型通过 DerefMut 特征的方法转换为 &mut U 类型，对应上例中，就是将 &mut MyBox<String> 转换为 &mut String
